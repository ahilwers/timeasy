...
export class ExternalIntegrationComponent implements OnInit {
    ...
    getProjectRefPlaceholder(): string {
        ...
        switch (provider) {
            ...
            case 'jira':
                return 'PROJECT-KEY';
            default:
                return this.translateService.instant('projects.external.projectRefPlaceholder');
        }
    }
    ...
    getProjectRefHelp(): string {
        ...
        switch (provider) {
            case 'github':
                return this.translateService.instant('projects.external.githubHelp');
            case 'gitlab':
                return this.translateService.instant('projects.external.gitlabHelp');
            case 'jira':
                return this.translateService.instant('projects.external.jiraHelp');
            default:
                return '';
        }
    }
    ...
    getConnectionMessage(): string {
        return this.translateService.instant('projects.external.connectedTo', {
        ...
        this.userAccountService.connectProjectToAccount(this.projectId, request).subscribe({
            next: (response) => {
                this.messageService.add({
                    ...
                    detail: this.translateService.instant('projects.external.connectSuccess')
                });
            ...
            error: (error) => {
                ...
                let errorMessage = this.translateService.instant('projects.external.connectError');

                // Provide more specific error messages based on error type
                if (error.status === 401) {
                    errorMessage = this.translateService.instant('projects.external.authError');
                } else if (error.status === 403) {
                    errorMessage = this.translateService.instant('projects.external.accessError');
                } else if (error.status === 503) {
                    errorMessage = this.translateService.instant('projects.external.providerError');
        ...
        this.userAccountService.disconnectProject(this.projectId).subscribe({
            next: (response) => {
                this.messageService.add({
                    severity: 'success',
                    summary: this.translateService.instant('success'),
                    detail: this.translateService.instant('projects.external.disconnectSuccess')
                });
            ...
            error: (error) => {
                this.messageService.add({
                    severity: 'error',
                    summary: this.translateService.instant('error'),
                    detail: error.error?.error || this.translateService.instant('projects.external.disconnectError')
                });
                this.isProcessing.set(false);
            }
        });
        ...
        this.externalService.syncProjectIssues(this.projectId).subscribe({
            next: (response) => {
                this.messageService.add({
                    severity: 'success',
                    summary: this.translateService.instant('success'),
                    detail: this.translateService.instant('projects.external.syncSuccess')
                });
                this.isSyncing.set(false);
            },
            error: (error) => {
                let errorMessage = this.translateService.instant('projects.external.syncError');

                // Provide more specific error messages based on error type
                if (error.status === 401) {
                    errorMessage = this.translateService.instant('projects.external.authError');
                } else if (error.status === 404) {
                    errorMessage = this.translateService.instant('projects.external.noConnectionError');
                } else if (error.status === 503) {
                    errorMessage = this.translateService.instant('projects.external.providerError');
                } else if (error.error?.error) {
                    errorMessage = error.error.error;
